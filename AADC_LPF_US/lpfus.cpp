...
tResult LPFUSFilter::OnPinEvent(IPin* pSource, tInt nEventCode, tInt nParam1, tInt nParam2, IMediaSample* pMediaSample)
{
	...
	if (nEventCode == IPinEventSink::PE_MediaSampleReceived)
	{
		if (pMediaSample != NULL && m_pCoderDescSignalInput != NULL)
		{
				...
				{
					outputSample[s]=0;
					for ( int i = 0; i< iAverageNumber; i++)
					{
						outputSample[s]+= (buffer[s][i]);
						//LOG_INFO(cString::Format("buffer %f", buffer[s][i]));
						
					}
					outputSample[s]=outputSample[s]/iAverageNumber;
                                        //LOG_INFO(cString::Format("outputsample [i] = %f", outputSample[0]));
				}
				
				//LOG_INFO(cString::Format("outputsample 1 %f", outputSample[0]));
...
tResult LPFUSFilter::TransmitOutput()
{
	...
	pCoderOutput->Set("tFrontCenter.f32Value", (tVoid*)&(outputSample[2]));
	pCoderOutput->Set("tFrontCenter.ui32ArduinoTimestamp", (tVoid*)&timeStamp);

	pCoderOutput->Set("tFrontCenterRight.f32Value", (tVoid*)&(outputSample[3]));
	pCoderOutput->Set("tFrontCenterRight.ui32ArduinoTimestamp", (tVoid*)&timeStamp);

	pCoderOutput->Set("tFrontRight.f32Value", (tVoid*)&(outputSample[4]));
	pCoderOutput->Set("tFrontRight.ui32ArduinoTimestamp", (tVoid*)&timeStamp);

	pCoderOutput->Set("tSideRight.f32Value", (tVoid*)&(outputSample[5]));
	pCoderOutput->Set("tSideRight.ui32ArduinoTimestamp", (tVoid*)&timeStamp);

	pCoderOutput->Set("tSideLeft.f32Value", (tVoid*)&(outputSample[6]));
